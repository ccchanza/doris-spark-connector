...
/** Creates per-client TLS trust material without changing JVM-global state. */
public final class DorisTlsContextFactory {
    ...
    public static SSLContext createSslContext(DorisTlsOptions options) {
        ...
        if (options.getCaCertificatePath().isEmpty()) {
            return defaultSslContext();
        }

        try {
            return createSslContext(createTrustStore(options.getCaCertificatePath()));
